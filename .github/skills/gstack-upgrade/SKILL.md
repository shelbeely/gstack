...
[ -z "$_AUTO" ] && _AUTO=$(~/.copilot/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.copilot/skills/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.copilot/skills/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.copilot/skills/gstack/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/.github/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.github/skills/gstack"
elif [ -d ".github/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".github/skills/gstack"
elif [ -d ".github/skills/gstack" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR=".github/skills/gstack"
elif [ -d "$HOME/.github/skills/gstack" ]; then
...
TMP_DIR=$(mktemp -d)
git clone --depth 1 https://github.com/shelbeely/gstack.git "$TMP_DIR/gstack"
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.github/skills/gstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.github/skills/gstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.github/skills/gstack"
  fi
fi
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.github/skills/gstack/` when you're ready."
...
```bash
~/.copilot/skills/gstack/bin/gstack-update-check --force 2>/dev/null || \
.github/skills/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.github/skills/gstack/` when you're ready."
